<template>
  <div class="edit-tag">
    ...
    <el-dialog title="工资管理" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="基本工资" :label-width="formLabelWidth">
          <el-input v-model="form.basic_salary" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="全勤工资" :label-width="formLabelWidth">
          <el-input v-model="form.full_attendance_pay" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="迟到扣款" :label-width="formLabelWidth">
          <el-input v-model="form.late_deduction" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="休假扣除" :label-width="formLabelWidth">
          <el-input v-model="form.leave_deduction" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="绩效奖金" :label-width="formLabelWidth">
          <el-input v-model="form.performance_bonus" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    submitEd() {
      let data = this.form
      this.axios.post('salary/insert', data, {
        headers: {
    ...
    handleSearch() {
      this.axios.get('salary/mocha/' + this.input, {
      })
        .then((response) => {
          let { data, status } = response
          this.$bus.$emit('searchSalary', data)
        })
        .catch(function (error) {
          console.log(error);
        })
