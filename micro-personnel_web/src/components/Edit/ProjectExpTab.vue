<template>
  <div class="paper-tab">
    ...
    <el-table id="elTab" :data="tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)" border style="width: 80%" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
      ...
      <el-table-column prop="duty" label="职责" width="180"></el-table-column>
    ...
    <el-dialog title="编辑项目经历" :visible.sync="dialogFormVisible">
      <el-form :model="form">
...
export default {
  ...
  methods: {
    ...
    subMit() {
      this.axios.put('proexp/update', this.form, {
        headers: {
  ...
  mounted() {
    ...
    if (userid != 1) {
      this.isShow = false
      this.$bus.$on("showProexp", (data) => {
        this.tableData.push(data)
        this.totalSize = data.length
        this.loading = false
      });
      this.$bus.$on('searchProexp', (data) => {
        console.log(data);
        this.tableData.push(data)
        this.totalSize = data.length
      })
